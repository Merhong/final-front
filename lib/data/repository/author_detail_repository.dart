...
class AuthorDetailRepository {
  ...
  Future<ResponseDTO> fetchInterestCreate(String jwt, int authorId) async {
    try {
      // 통신
  ...
  Future<ResponseDTO> fetchInterestDelete(String jwt, int authorId) async {
    try {
      // 통신
      Response response = await dio.delete("/authors/interest/$authorId", options: Options(headers: {"Authorization": "${jwt}"}));
