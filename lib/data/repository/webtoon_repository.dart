...
//
  ...
  Future<ResponseDTO> fetchAdvertisingSubList(String jwt) async {
    try {
      // 통신
      Response response = await dio.get("/webtoons/advertising/sub", options: Options(headers: {"Authorization": "${jwt}"}));

      // 응답 받은 데이터 파싱
      ResponseDTO responseDTO = new ResponseDTO.fromJson(response.data);
      List<dynamic> mapList = responseDTO.data as List<dynamic>;

      List<AdvertisingSubDTO> asDTOList = mapList.map((asDTO) => AdvertisingSubDTO.fromJson(asDTO)).toList();
      responseDTO.data = asDTOList;
